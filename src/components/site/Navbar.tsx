...
export function Navbar() {
  ...
  return (
    <header className={`sticky top-0 z-40 transition-all ${scrolled ? "border-b border-border/60 bg-background/85 backdrop-blur-xl shadow-soft" : "bg-background/40 backdrop-blur"}`}>
      {/* Top utility bar */}
      <div className="hidden md:block bg-gradient-to-r from-navy via-brand-800 to-navy text-white/90 text-[11px]">
        <div className="container-page flex items-center justify-between h-8">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5"><Phone className="h-3 w-3 text-gold" /> +91 98307 73655</span>
      ...
      <div className="container-page flex h-16 items-center justify-between">
        <a href="#top" className="flex items-center gap-2.5">
          ...
          <div className="leading-tight">
            ...
            <div className="text-[10px] uppercase tracking-[0.18em] text-brand font-semibold">Taxation Mentor · VBTC</div>
          </div>
        </a>
        ...
        <nav className="hidden lg:flex items-center gap-7">
          ...
          >
            ...
            >
              Courses <ChevronDown className={`h-3.5 w-3.5 transition-transform ${coursesOpen ? "rotate-180" : ""}`} />
            </button>
            {coursesOpen && (
              <div className="absolute left-1/2 -translate-x-1/2 top-full pt-3 w-[680px] animate-fade-up">
                <div className="rounded-3xl border border-border bg-background/95 backdrop-blur-xl shadow-elevated p-3 grid grid-cols-3 gap-2">
                  {courseGroups.map(g => (
                    ...
                    >
                      <div className={`grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br ${g.color} text-white shadow-soft`}>
                        <g.icon className="h-5 w-5" />
                      </div>
                      <div className="mt-3 font-display text-base font-bold text-foreground">{g.title}</div>
                      <div className="text-[11px] text-brand-700 font-medium">{g.subtitle}</div>
                      <ul className="mt-3 space-y-1.5">
                        {g.items.map(it => (
                          <li key={it.label} className="text-[12px] text-foreground/75 flex items-start justify-between gap-2 leading-tight">
                            <span>{it.label}</span>
                            <span className="text-brand font-semibold whitespace-nowrap">{it.price}</span>
                          </li>
                        ))}
                      </ul>
                    </a>
                  ))}
                  <a href="#courses" onClick={() => setCoursesOpen(false)} className="col-span-3 mt-1 rounded-2xl bg-gradient-to-r from-brand to-indigo-600 text-white px-4 py-3 flex items-center justify-between text-sm font-semibold hover:shadow-brand transition-shadow">
        ...
        <div className="hidden lg:flex items-center gap-3">
          <a href="https://wa.me/919051903915" target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-emerald-600 hover:text-emerald-700 inline-flex items-center gap-1.5">
            <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" /> WhatsApp
          </a>
          <Button className="bg-gradient-to-r from-brand to-indigo-600 text-white hover:shadow-brand">Enroll Now</Button>
        </div>
      ...
      {open && (
        <div className="lg:hidden border-t border-border bg-background animate-fade-up">
          <div className="container-page py-4 flex flex-col gap-1">
            <div className="text-[10px] uppercase tracking-widest text-brand font-bold pt-2 pb-1">Courses</div>
            {courseGroups.map(g => (
              <a key={g.title} href="#courses" onClick={() => setOpen(false)} className="flex items-center gap-3 py-2.5 rounded-xl hover:bg-brand-50 px-2">
                <div className={`grid h-9 w-9 place-items-center rounded-lg bg-gradient-to-br ${g.color} text-white`}>
            ...
            <Button className="bg-gradient-to-r from-brand to-indigo-600 text-white mt-2">Enroll Now</Button>
          </div>
        </div>
      )}
    </header>
